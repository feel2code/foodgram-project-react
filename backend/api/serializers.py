...
class CommonRecipe(metaclass=serializers.SerializerMetaclass):
    ...
    def get_is_favorite(self, obj):
        ...
        if request.user.is_anonymous:
            return False
        elif Favorite.objects.filter(user=request.user, recipe__id=obj.id).exists():
            return True
        return False
...
class IngredientSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        extra_kwargs = {'name': {'required': False}, 'measurement_unit': {'required': False}}
...
class TagSerializer(serializers.ModelSerializer):
    class Meta:
        model = Tag
        fields = '__all__'
        extra_kwargs = {'name': {'required': False}, 'slug': {'required': False}, 'color': {'required': False}}
...
class RecipeSerializer(serializers.ModelSerializer, CommonRecipe):
    ...
    ingredients = IngredientAmountSerializer(source='ingredient_recipes', many=True)
    is_in_shopping_cart = serializers.SerializerMethodField()

    class Meta:
        model = Recipe
        fields = (
            'id', 'author', 'name', 'image', 'text', 'ingredients', 'tags', 'cooking_time', 'is_in_shopping_cart',
            'is_favorite'
        )
...
class RecipeSerializerPost(serializers.ModelSerializer, CommonRecipe):
    ...
    tags = serializers.PrimaryKeyRelatedField(
        ...
        many=True)
    ingredients = IngredientAmountRecipeSerializer(source='ingredient_recipes', many=True)
    image = Base64ImageField(max_length=None, use_url=False,)
    ...
    def update(self, instance, validated_data):
        ...
        instance = self.add_tags_and_ingredients(tags_data, ingredients, instance)
...
class SubscriptionSerializer(serializers.ModelSerializer, CommonSubscribed, CommonCount):
    ...
    def get_recipes(self, obj):
        ...
        if request.GET.get('recipes_limit'):
            ...
            queryset = Recipe.objects.filter(author__id=obj.id).order_by('id')[:recipes_limit]
        else:
